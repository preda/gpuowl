...
typedef TT T2;
Word2 OVERLOAD carryPair(T2 u, i32 *outCarry, bool b1, bool b2, i32 inCarry, float *maxROE, float* carryMax) {
i32 midCarry;
...
Word2 OVERLOAD carryPair(T2 u, i64 *outCarry, bool b1, bool b2, i64 inCarry, float *maxROE, float* carryMax) {
i64 midCarry;
Word a = carryStep(doubleToLong(u.x, maxROE) + inCarry, &midCarry, b1);
Word b = carryStep(doubleToLong(u.y, maxROE) + midCarry, outCarry, b2);
// #if STATS & 0x5
...
KERNEL(G_W) kernCarryA(u32 posROE, P(Word2) out, CP(T2) in, P(CarryABM) carryOut, CP(u32) bits, P(uint) ROE) {
u32 g  = get_group_id(0);
u32 me = get_local_id(0);
u32 gx = g % NW;
u32 gy = g / NW;
...
KERNEL(G_W) kernCarryM(u32 posROE, P(Word2) out, CP(T2) in, P(CarryABM) carryOut, CP(u32) bits, P(uint) ROE) {
u32 g  = get_group_id(0);
u32 me = get_local_id(0);
u32 gx = g % NW;
u32 gy = g / NW;
...
KERNEL(G_W) kernCarryFused(u32 posROE, P(T2) out, CP(T2) in, P(i64) carryShuttle, P(u32) ready, Trig smallTrig,
...
KERNEL(G_W) kernCarryFusedMul(u32 posROE, P(T2) out, CP(T2) in, P(i64) carryShuttle, P(u32) ready, Trig smallTrig,
CP(u32) bits, P(uint) ROE) {
local T2 lds[WIDTH / 2];
u32 gr = get_group_id(0);
u32 me = get_local_id(0);
